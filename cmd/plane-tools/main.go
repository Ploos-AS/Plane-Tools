...
func main() {
	...
	if err := configureAircraftStore(aircraftCSV); err != nil { log.Fatalf("load aircraft dataset: %v", err) }
	airportsCSV := getenv("PLANE_TOOLS_AIRPORTS_CSV", "/data/airports.csv")
	runwaysCSV := getenv("PLANE_TOOLS_RUNWAYS_CSV", "/data/runways.csv")
	if err := configureAirportStore(airportsCSV, runwaysCSV); err != nil { log.Fatalf("load airport dataset: %v", err) }
	spottingLocationsPath := getenv("PLANE_TOOLS_SPOTTING_LOCATIONS", "/data/spotting-locations.json")
	if err := configureSpottingLocationStore(spottingLocationsPath); err != nil { log.Fatalf("load spotting locations: %v", err) }
	spottingLogPath := getenv("PLANE_TOOLS_SPOTTING_LOG", "/data/spotting-log.json")
	if err := configureSpottingLogStore(spottingLogPath); err != nil { log.Fatalf("load spotting log: %v", err) }
	...
	if raw := strings.TrimSpace(os.Getenv("PLANE_TOOLS_ADSB_TIMEOUT")); raw != "" {
		...
		if err != nil { log.Fatalf("parse PLANE_TOOLS_ADSB_TIMEOUT: %v", err) }
		adsbTimeout = parsed
	}
	if err := configureADSBReceiver(adsbURL, adsbTimeout); err != nil { log.Fatalf("configure ADS-B receiver: %v", err) }
	if err := configureADSBPosition(os.Getenv("PLANE_TOOLS_ADSB_LAT"), os.Getenv("PLANE_TOOLS_ADSB_LON")); err != nil {
		log.Fatalf("configure ADS-B receiver position: %v", err)
	}
	...
	mux.HandleFunc("GET /api/v1/adsb/aircraft/search", adsbAircraftSearchHandler)
...
func distanceHandler(w http.ResponseWriter, r *http.Request) {
	lat1, err := queryFloat(r, "lat1"); if err != nil { writeError(w, err); return }
	lon1, err := queryFloat(r, "lon1"); if err != nil { writeError(w, err); return }
	lat2, err := queryFloat(r, "lat2"); if err != nil { writeError(w, err); return }
	lon2, err := queryFloat(r, "lon2"); if err != nil { writeError(w, err); return }
	if !validLatLon(lat1, lon1) || !validLatLon(lat2, lon2) { writeError(w, fmt.Errorf("coordinates must use latitude -90..90 and longitude -180..180")); return }
	writeJSON(w, http.StatusOK, distanceResponse{DistanceNM: round(greatCircleNM(lat1, lon1, lat2, lon2), 3), BearingDeg: round(initialBearing(lat1, lon1, lat2, lon2), 2)})
}

func convertHandler(w http.ResponseWriter, r *http.Request) {
	value, err := queryFloat(r, "value"); if err != nil { writeError(w, err); return }
	from := strings.ToLower(strings.TrimSpace(r.URL.Query().Get("from")))
	to := strings.ToLower(strings.TrimSpace(r.URL.Query().Get("to")))
	result, err := convert(value, from, to); if err != nil { writeError(w, err); return }
...
func initialBearing(lat1, lon1, lat2, lon2 float64) float64 {
	phi1, phi2 := radians(lat1), radians(lat2); dLambda := radians(lon2-lon1)
...
func convert(value float64, from, to string) (float64, error) {
	if from == to && from != "" { return value, nil }
	const nmToKM = 1.852; const knotToKPH = 1.852; const footToM = 0.3048
	switch from+":"+to {
	case "nm:km": return value*nmToKM, nil
	case "km:nm": return value/nmToKM, nil
	case "kt:kph": return value*knotToKPH, nil
	case "kph:kt": return value/knotToKPH, nil
	case "ft:m": return value*footToM, nil
	case "m:ft": return value/footToM, nil
	default: return 0, fmt.Errorf("unsupported conversion %q to %q; supported: nm/km, kt/kph, ft/m", from, to)
	}
}

func queryFloat(r *http.Request, name string) (float64, error) {
	raw := strings.TrimSpace(r.URL.Query().Get(name)); if raw == "" { return 0, fmt.Errorf("missing query parameter %q", name) }
	value, err := strconv.ParseFloat(raw, 64); if err != nil { return 0, fmt.Errorf("invalid %s: %w", name, err) }
	return value, nil
}

func validLatLon(lat, lon float64) bool { return lat >= -90 && lat <= 90 && lon >= -180 && lon <= 180 }
func radians(v float64) float64 { return v * math.Pi / 180 }
func round(v float64, places int) float64 { scale := math.Pow10(places); return math.Round(v*scale)/scale }
func writeError(w http.ResponseWriter, err error) { writeJSON(w, http.StatusBadRequest, map[string]string{"error": err.Error()}) }
